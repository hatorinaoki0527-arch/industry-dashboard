...
function historyNeeds(){
 ...
 if(state.view==='sector'&&jpCalendar.key===state.sector+'|'+state.date){
  const chosen=jpCalendar.date||dates.filter(d=>d.startsWith(jpCalendar.month)&&d<=state.date).at(-1);
  if(chosen)wanted.push(chosen);
 }
 const missing=new Set(wanted.filter(d=>byDate.get(d)&&!byDate.get(d).rowsLoaded).map(d=>d.slice(0,7)));
 const needed=DATA.historyChunks.filter(c=>missing.has(c.month));
 if(state.view==='stock'&&!stockPrices.has(state.stock)){
  const part=DATA.priceChunks?.find(c=>c.code===state.stock);if(part)needed.push({...part,kind:'prices'});
 }
 return needed;
}
function prepareHistory(){
 const needed=historyNeeds();if(!needed.length)return false;
 if(historyError){$('content').innerHTML='<section class="panel"><h3>历史数据尚未加载</h3><p>'+esc(historyError)+'</p><button class="button" data-history-retry>重新加载</button></section>';return true;}
 $('content').innerHTML='<section class="panel"><h3>正在加载所选历史</h3><p>按需读取 '+needed.length+' 个加密历史文件，已载入的文件会复用；日K长历史仅加载所选股票。</p></section>';
 if(historyBusy)return true;
 historyBusy=true;const attempt=epoch,data=DATA,key=historyKey,salt=historySalt;
 (async()=>{
  for(const part of needed){
   if(attempt!==epoch||DATA!==data)return;
   if(!(part.kind==='prices'?/^prices\/[a-zA-Z0-9]{2}\/[a-zA-Z0-9]{1,12}\.[a-f0-9]{16}\.json$/:/^history\/japan-\d{4}-\d{2}\.[a-f0-9]{16}\.json$/).test(part.file))throw Error('历史文件路径无效');
   const loaded=await DashboardVault.load({url:new URL(part.file,location.href).href,timeoutMs:60000});
   const v=loaded.vault;if(v.salt!==salt)throw Error('历史文件版本不匹配，请重新进入网站');
   ...
   const expected=data.days.filter(d=>d.date.startsWith(part.month)).map(d=>d.date);
   if(body.schemaVersion!==1||body.market!=='japan'||body.month!==part.month||!Array.isArray(body.days)||body.days.length!==expected.length||new Set(body.days.map(d=>d.date)).size!==expected.length||body.days.some(d=>!expected.includes(d.date)||!Array.isArray(d.rows)||d.rows.some(r=>!Array.isArray(r)||r.length!==data.columns.length)))throw Error('历史记录校验失败');
   if(attempt!==epoch||DATA!==data)return;
   for(const item of body.days){const target=byDate.get(item.date);target.rows=item.rows;target.records=item.rows.map(rowObj);target.rowsLoaded=true;for(const r of target.records){if(!byStock.has(r.code))byStock.set(r.code,new Map());byStock.get(r.code).set(item.date,r);}}
   await loaded.persist();
  }
 })().catch(()=>{if(attempt===epoch&&DATA===data)historyError='无法读取或验证所选历史。已载入数据保留，请联网后重试；旧日期不会用今天的数据代替。';}).finally(()=>{if(attempt===epoch&&DATA===data){historyBusy=false;render();}});
 return true;
}
document.addEventListener('click',e=>{if(e.target.closest('[data-history-retry]')){historyError='';render();}});
...
const maSelected={price:new Set([5,20,60]),amount:new Set([5,20]),change:new Set([5,20,60])};
document.addEventListener('change',e=>{const box=e.target;if(!box.matches('[data-ma-period]'))return;const panel=box.closest('[data-ma-kind]'),kind=panel.dataset.maKind,n=Number(box.dataset.maPeriod);if(box.checked)maSelected[kind].add(n);else maSelected[kind].delete(n);panel.querySelectorAll('[data-ma-line]').forEach(line=>{line.style.display=maSelected[kind].has(Number(line.dataset.maLine))?'':'none';});});
...
function trendPanel(points,key,label,cutoff,format=fmt){
 ...
 const periods=[5,10,20,60,90], colors=['#a97000','#8556ba','#267aa0','#d16435','#576a7f'];
 const kind=key==='amount'?'amount':key==='change'?'change':'price', selected=maSelected[kind];
 const averages=periods.map(n=>rollingValues(all,key,n));
 const offset=Math.max(0,all.length-120), rows=all.slice(offset), curves=averages.map(a=>a.slice(offset));
 ...
 return `<section class="panel" data-ma-kind="${kind}"><h3>${esc(label)}</h3><div class="ma-toggles">${periods.map((n,j)=>`<label style="color:${colors[j]}"><input type="checkbox" data-ma-period="${n}" ${selected.has(n)?'checked':''}><span>${n}日${amount?'均额':kind==='change'?'涨跌均值':'均线'}<b>${valid(averages[j].at(-1))?esc(format(averages[j].at(-1))):'样本不足（'+all.slice(-n).filter(r=>valid(r[key])).length+'/'+n+'）'}</b></span></label>`).join('')}</div>${amount?`<p>当日 / 此前20日均额：<b>${valid(ratio)?fmt(ratio)+'×':'—（样本不足或均额为零）'}</b> · 成交活跃度，不是资金净流入</p>`:''}<div class="chart"><svg viewBox="0 0 ${W} ${H}" role="img" aria-label="${esc(label)}与5、10、20、60、90日平均线">${Array.from({length:5},(_,i)=>{const v=lo+(hi-lo)*i/4;return `<line x1="${L}" x2="${W-R}" y1="${y(v)}" y2="${y(v)}" stroke="#e0e7e1"/><text x="${L-6}" y="${y(v)+4}" text-anchor="end" font-size="11">${esc(format(v))}</text>`;}).join('')}${raw}${curves.map((v,j)=>`<path data-ma-line="${periods[j]}" style="display:${selected.has(periods[j])?'':'none'}" d="${path(v)}" fill="none" stroke="${colors[j]}" stroke-width="2"/>`).join('')}${rows.map((r,i)=>valid(r[key])?`<circle cx="${x(i)}" cy="${y(r[key])}" r="3" fill="#354b43"><title>${esc(r.date)}：${esc(format(r[key]))}${periods.map((n,j)=>'；'+n+'日 '+(valid(curves[j][i])?format(curves[j][i]):'样本不足')).join('')}</title></circle>`:'').join('')}${[...new Set([0,Math.floor(rows.length/2),rows.length-1])].map(i=>`<text x="${x(i)}" y="${H-8}" text-anchor="middle" font-size="11">${esc(rows[i].date)}</text>`).join('')}</svg></div><p class="chart-note">图中显示最近120个归档交易日，计算使用已载入历史；平均线含当日。勾选切换曲线；成交额倍数的20日基准不含当日。缺失值使对应窗口留空；按归档交易日计算，不代表盘中量比。</p><details><summary>展开均线明细</summary><div class="data-scroll"><table><thead><tr><th>日期</th><th>当日值</th>${periods.map(n=>`<th>${n}日平均</th>`).join('')}</tr></thead><tbody>${rows.map((r,i)=>`<tr><td>${esc(r.date)}</td><td>${esc(format(r[key]))}</td>${curves.map(c=>`<td>${esc(format(c[i]))}</td>`).join('')}</tr>`).reverse().join('')}</tbody></table></div></details></section>`;
}
...
function jpCandleHistory(code,fallback){const map=new Map((stockPrices.get(code)||fallback).map(r=>[r.date,r]));return dates.map(date=>({date,...(map.get(date)||{})}));}
function candlePanel(points,cutoff,label){return CandleCharts.render(points,{cutoff,years:kYears,label,selected:maSelected.price});}
document.addEventListener('click',e=>{const b=e.target.closest('[data-k-years]');if(!b||!DATA)return;const n=Number(b.dataset.kYears);if(![1,2,3,4,5].includes(n))return;kYears=n;const top=b.closest('.candle-panel').getBoundingClientRect().top;render();const panel=document.querySelector('.candle-panel');if(panel)window.scrollBy(0,panel.getBoundingClientRect().top-top);});
function cnIndustryTrends(code,days,cutoff){
 ...
 return {date:d.date,open:index?.open,high:index?.high,low:index?.low,close:index?.level,level:index?.level??null,amount:rs.length&&rs.every(r=>valid(r.amount))?rs.reduce((sum,r)=>sum+r.amount,0):null};
 });
 return candlePanel(points,cutoff,'行业指数 · 日K线（点）')+trendPanel(points,'amount','已采集成分股成交额 · 均额线',cutoff,money)+'<p class="chart-note">成交额为当日已采集成分股真实金额之和，非官方行业总额；缺失样本和成分变化会影响可比性。不同市场不混合计算。</p>';
}
...
function cnExclusionNotice(day){
 const rows=day?.excludedStocks||[];
 if(!rows.length)return '';
 return `<details class="notice"><summary>未纳入排行 ${rows.length}只 · 查看原因</summary><p>以下股票保留所属行业，但无有效当日成交记录，不以0涨跌补位。历史接口缺少日线不直接等同于停牌。</p>${rows.map(r=>`<p><strong>${esc(r.name)} · ${esc(r.code)}</strong>：${esc(r.reason)}${r.lastAvailableDate?'；历史源最后记录 '+esc(r.lastAvailableDate):''}</p>`).join('')}</details>`;
}
...
function renderUS(){
 ...
 $('status').innerHTML=china?cnUpdateStatus(bundle,d,DATA.cnStocks):`<div class="status ${bundle.status.state==='success'?'':'warn'}"><b>${bundle.status.state==='success'?'● 已读取来源日期快照':bundle.status.state==='waiting_for_close'?'● 等待美股收盘日期快照，保留上次数据':'● 取数异常，保留旧快照'}</b><span>来源日期 ${esc(d.date)} · ${marketName}市场</span><span>归档 ${days.length} 日 · ${count} 个指数</span></div>`;
 ...
 if(!china&&selected)detail=trendPanel(days.filter(d=>d.date<=usDate).map(d=>({date:d.date,level:d.rows.find(r=>r.code===selected.code)?.level??null})),'level','行业指数 · 价格均线',usDate)+detail;
