...
function industryDateTree(history,code,china){
 ...
 const cells=Array.from({length:offset},()=>'<div class="calendar-pad" aria-hidden="true"></div>');
 for(let n=1;n<=nDays;n++){
  const date=cnCalendarMonth+'-'+String(n).padStart(2,'0'),x=byDay.get(date),weekend=(offset+n-1)%7>=5;
  cells.push(x?`<button type="button" class="calendar-day ${tone(x.row.change)} ${date===cnCalendarDate?'selected':''}" data-cn-calendar-date="${date}" aria-pressed="${date===cnCalendarDate}" aria-label="${date} ${pct(x.row.change)}"><span>${n}${date===latest?'<small>最新</small>':''}</span><strong>${pct(x.row.change)}</strong></button>`:`<div class="calendar-day no-record"><span>${n}</span><small>${date>latest?'未纳入':weekend?'周末':'无归档'}</small></div>`);
 }
