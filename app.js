...
function lock(){epoch++;historyKey=null;historySalt='';historyBusy=false;historyError='';DATA=null;dates=[];byDate.clear();byStock.clear();$('content').replaceChildren();$('heading').replaceChildren();$('controls').replaceChildren();$('status').replaceChildren();$('workspace').hidden=true;$('gate').hidden=false;$('password').value='';$('password').focus();}
...
function historyNeeds(){
 ...
 return DATA.historyChunks.filter(c=>missing.has(c.month));
}
function prepareHistory(){
 ...
 $('content').innerHTML='<section class="panel"><h3>正在加载所选历史</h3><p>按需读取 '+needed.length+' 个月的加密个股记录，已载入的月份会复用。</p></section>';
 ...
 (async()=>{
  for(const part of needed){
   ...
   if(!/^history\/japan-\d{4}-\d{2}\.[a-f0-9]{16}\.json$/.test(part.file))throw Error('历史文件路径无效');
 ...
 })().catch(()=>{if(attempt===epoch&&DATA===data)historyError='无法读取或验证所选月份。已载入数据保留，请联网后重试；旧日期不会用今天的数据代替。';}).finally(()=>{if(attempt===epoch&&DATA===data){historyBusy=false;render();}});
 return true;
}
...
function cnIndustryTrends(code,days,cutoff){
 ...
 return trendPanel(points,'level','行业指数 · 价格均线',cutoff)+trendPanel(points,'amount','已采集成分股成交额 · 均额线',cutoff,money)+'<p class="chart-note">成交额为当日已采集成分股真实金额之和，非官方行业总额；缺失样本和成分变化会影响可比性。不同市场不混合计算。</p>';
}
